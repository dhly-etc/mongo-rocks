    ...
    public:
        ...
        Status init() {
            ...
            if (_mongoRateLimiter.get() == nullptr) {
                _mongoRateLimiter = std::make_unique<MongoRateLimiter>(
                    rocksdb::NewGenericRateLimiter(kInitMongoRateLimitRequestTokens));
            }
            ...
            DiskStats currStats(
                readStatus.getValue().getField(getMongoRateLimitParameter().getDisk()).Obj());
        ...
        void run() {
            while (!globalInShutdownDeprecated()) {
                ...
                if (ratioStatus.isOK()) {
                    int64_t newTokens = static_cast<int64_t>(
                        _mongoRateLimiter->getTokensPerSecond() * ratioStatus.getValue());
        ...
        StatusWith<double> calculateResetRatio(bool exhausted, DiskStats& currStats) {
            ...
            if (currStats.reads >= _prevStats.reads && currStats.writes >= _prevStats.writes) {
                iops = static_cast<uint64_t>(((currStats.reads - _prevStats.reads) +
                                              (currStats.writes - _prevStats.writes)) *
                                             ratio);
            }
            if (currStats.read_sectors >= _prevStats.read_sectors &&
                ...
                kbps =
                    static_cast<uint64_t>(((currStats.read_sectors - _prevStats.read_sectors) +
                                           (currStats.write_sectors - _prevStats.write_sectors)) *
                                          ratio / 2);
            }
            if (iops == 0 || kbps == 0) {
                return Status(ErrorCodes::InternalError, str::stream()
                                                             << "do not reset: iops is " << iops
                                                             << "; kbps is " << kbps);
            }
            double resetRatio = std::min(
                getMongoRateLimitParameter().getIops() / static_cast<double>(iops),
            ...
            return Status(ErrorCodes::InternalError, str::stream()
                                                         << "do not reset: not meet condition");
        }

        MongoRateLimiter* getMongoRateLimiter() { return _mongoRateLimiter.get(); }
