...
namespace rocksdb {
    ...
    namespace {
        ...
        size_t TxnKeySize(const TxnKey& key) { return sizeof(key.first) + key.second.size(); }
    ...
    const TxnKey PrepareHeap::sentinal_ = std::make_pair(std::numeric_limits<uint32_t>::max(), "");
    ...
    std::shared_ptr<ATN> PrepareHeap::Find(const TOTransactionImpl::ActiveTxnNode* core,
                                           const TxnKey& key,
    ...
    uint32_t PrepareHeap::Remove(ATN* core) {
        ...
        auto iter_cb = [&](uint32_t cf_id, const Slice& key) {
            ...
            auto it = map_.find(lookup_key);
            if (it != map_.end()) {
                invariant(!it->second.empty() && it->second.front()->txn_id_ == core->txn_id_);
                it->second.pop_front();
                cnt++;
                if (it->second.empty()) {
                    map_.erase(it);
    ...
    Status TOTransactionDB::Open(const Options& options,
        ...
        Status s = Open(db_options, txn_db_options, dbname, column_families, &handles, trim_cfds,
                        trimHistory, stable_ts_key, dbptr);
        if (s.ok()) {
    ...
    Status TOTransactionDB::Open(const DBOptions& db_options,
        ...
        for (const auto& cfd : open_cfds) {
            if (cfd.options.comparator == nullptr ||
                cfd.options.comparator->timestamp_size() == 0) {
        ...
        if (trimHistory) {
            ...
            // step4: close db
            if (s.ok()) {
                LOGV2_DEBUG(0, 0, "TOTDB recover to stableTs",
                            "ts"_attr = rocksdb::Slice(trim_ts).ToString(true));
                std::vector<ColumnFamilyHandle*> tmp_cfhs;
                DB* tmp_db = nullptr;
                s = DB::OpenAndTrimHistory(db_options, dbname, trim_cfds, &tmp_cfhs, &tmp_db,
                                           trim_ts);
                if (!s.ok()) {
                    return s;
                }
        ...
        LOGV2_DEBUG(0, 0, "TOTDB open on normal mode");
        DB* db = nullptr;
        ...
        LOGV2_DEBUG(0, 0, "TOTDB open success");
        return s;
    }

    Status TOTransactionDBImpl::UnCommittedKeys::RemoveKeyInLock(const TxnKey& key,
        ...
        auto ccbytes =
            mem_usage->fetch_sub(TxnKeySize(key) + sizeof(iter->second), std::memory_order_relaxed);
        invariant(ccbytes >= 0);
        (void)ccbytes;
    ...
    Status TOTransactionDBImpl::UnCommittedKeys::CheckKeyAndAddInLock(
        ...
        if (iter != stripe->uncommitted_keys_map_.end()) {
            ...
            if (iter->second != txn_id) {
                LOGV2_DEBUG(0, 2, "TOTDB WriteConflict another txn is modifying key",
                            "txnId"_attr = iter->second,
                            "key"_attr = rocksdb::Slice(key.second).ToString(true));
                return Status::Busy();
            } else {
                return Status::OK();
            }
    ...
    Status TOTransactionDBImpl::CommittedKeys::AddKeyInLock(const TxnKey& key,
        ...
        if (stripe->committed_keys_map_.find(key) == stripe->committed_keys_map_.end()) {
            mem_usage->fetch_add(TxnKeySize(key) + sizeof(prepare_ts) + sizeof(commit_ts) +
                                 sizeof(commit_txn_id));
        }

        stripe->committed_keys_map_[key] = std::make_tuple(commit_txn_id, prepare_ts, commit_ts);

        return Status::OK();
    }

    Status TOTransactionDBImpl::CommittedKeys::CheckKeyInLock(const TxnKey& key,
        ...
        if (iter != stripe->committed_keys_map_.end()) {
            ...
            if (committed_txn_id > txn_id) {
                LOGV2_DEBUG(0, 2, "TOTDB WriteConflict a committed txn greater than mine",
                            "committed"_attr = committed_txn_id, "mine"_attr = txn_id);
                return Status::Busy("SI conflict");
            }
            // Find the latest committed txn for this key and its commit ts
            if (committed_ts > timestamp) {
                LOGV2_DEBUG(0, 2, "TOTDB WriteConflict a committed txn greater than my read_ts",
                            "committed"_attr = committed_ts, "read_ts"_attr = timestamp);
    ...
    TOTransaction* TOTransactionDBImpl::BeginTransaction(const WriteOptions& write_options,
        ...
        TOTransaction* newTransaction = nullptr;
        {
            ...
            newActiveTxnNode->timestamp_round_prepared_ = txn_options.timestamp_round_prepared;
        ...
        LOGV2_DEBUG(0, 2, "TOTDB begin a txn", "txn_id"_attr = newActiveTxnNode->txn_id_,
                    "snapshot"_attr = newActiveTxnNode->txn_snapshot->GetSequenceNumber());
        return newTransaction;
    }

    Status TOTransactionDBImpl::CheckWriteConflict(const TxnKey& key, const TransactionID& txn_id,
                                                   const RocksTimeStamp& readts) {
        // if first check the uc key and ck busy ,it will need remove uc key ,so we check commit key
        // first
        ...
        if (!s.ok()) {
            LOGV2_DEBUG(0, 2, "TOTDB txn conflict ck", "txn_id"_attr = txn_id,
                        "key"_attr = rocksdb::Slice(key.second).ToString(true));
            return s;
        }
        ...
        s = uncommitted_keys_.CheckKeyAndAddInLock(
            key, txn_id, stripe_num, max_conflict_bytes_.load(std::memory_order_relaxed),
            &current_conflict_bytes_);
        if (!s.ok()) {
            LOGV2_DEBUG(0, 2, "TOTDB txn conflict uk", "txn_id"_attr = txn_id,
                        "key"_attr = rocksdb::Slice(key.second).ToString(true));
            return s;
        }
                    ...
                    while (map_iter != stripe->committed_keys_map_.end()) {
                        ...
                        if (txn_id <= txn && (commit_ts < ts || commit_ts == 0)) {
                            auto ccbytes = current_conflict_bytes_.fetch_sub(
                                TxnKeySize(map_iter->first) + sizeof(txn_id) + sizeof(prepare_ts) +
                                    sizeof(commit_ts),
    ...
    Status TOTransactionDBImpl::GetConsiderPrepare(const std::shared_ptr<ATN>& core,
                                                   ReadOptions& options,
                                                   ColumnFamilyHandle* column_family,
                                                   const Slice& key, std::string* value) {
        RocksTimeStamp read_ts =
            core->read_ts_set_ ? core->read_ts_ : std::numeric_limits<RocksTimeStamp>::max();
        (void)read_ts;
        const auto lookup_key =
            std::make_pair(column_family->GetID(), std::string(key.data(), key.size()));
        TOTransaction::TOTransactionState state;
        ...
        if (it == nullptr) {
            return core->write_batch_.GetFromBatchAndDB(GetRootDB(), options, column_family, key,
                                                        value);
        }
        ...
        if (state == TOTransaction::TOTransactionState::kPrepared) {
            ...
            if (core->ignore_prepare_) {
                ...
                return core->write_batch_.GetFromBatchAndDB(GetRootDB(), options, column_family,
                                                            key, value);
            }
    ...
    Iterator* TOTransactionDBImpl::NewIteratorConsiderPrepare(const std::shared_ptr<ATN>& core,
                                                              ColumnFamilyHandle* column_family,
                                                              Iterator* db_iter) {
        ...
        auto merge_iter = std::unique_ptr<PrepareMergingIterator>(
            new PrepareMergingIterator(std::move(base_writebatch), std::move(pmap_iter)));
    ...
    Status TOTransactionDBImpl::PublushTimeStamp(const std::shared_ptr<ATN>& core) {
        ...
        if (core->commit_ts_set_) {
             ...
             */
            if (core->state_.load(std::memory_order_relaxed) == TOTransaction::kPrepared) {
                return Status::OK();
            }
            ts = core->commit_ts_;
        } else {
...

        {
            ...
            for (auto it = commit_q_.begin(); it != commit_q_.end();) {
                ...
                if (state == TOTransaction::kStarted || state == TOTransaction::kPrepared) {
                    break;
                }
                // TODO: add walk stat
    ...
    Status TOTransactionDBImpl::SetDurableTimeStamp(const std::shared_ptr<ATN>& core,
                                                    const RocksTimeStamp& ts) {
        if (ts == 0) {
        ...
        if (has_oldest && ts < tmp_oldest) {
            return Status::InvalidArgument("durable timestamp is less than oldest timestamp");
        }
        if (ts < core->commit_ts_) {
            return Status::InvalidArgument("durable timestamp is less than commit timestamp");
        }
        core->durable_ts_ = ts;
    ...
    Status TOTransactionDBImpl::TxnAssertAfterReads(const std::shared_ptr<ATN>& core,
                                                    const char* op,
                                                    const RocksTimeStamp& timestamp) {
        uint64_t walk_cnt = 0;
        std::shared_lock<std::shared_mutex> rl(read_ts_mutex_);
        for (auto it = read_q_.rbegin(); it != read_q_.rend(); it++) {
            ...
            if (it->second->read_ts_ >= timestamp) {
                LOGV2_DEBUG(0, 2, "op timestamp  must be greater than the latest active readts",
                            "op"_attr = op, "timestamp"_attr = timestamp,
                            "active readts"_attr = it->second->read_ts_);
                return Status::InvalidArgument(
    ...
    Status TOTransactionDBImpl::PrepareTransaction(const std::shared_ptr<ATN>& core) {
    ...
    Status TOTransactionDBImpl::SetPrepareTimeStamp(const std::shared_ptr<ATN>& core,
                                                    const RocksTimeStamp& timestamp) {
        invariant(!core->prepare_ts_set_);
        ...
        if (timestamp < tmp_oldest) {
            if (core->timestamp_round_prepared_) {
                ...
                LOGV2_DEBUG(0, 2, "TOTDB round txn to oldest", "txn_id"_attr = core->txn_id_,
                            "prepare_ts"_attr = timestamp, "oldest"_attr = tmp_oldest);
                return Status::OK();
            } else {
                return Status::InvalidArgument("prepareTs should not be less than oldest ts");
            }
        } else {
    ...
    Status TOTransactionDBImpl::CommitTransaction(const std::shared_ptr<ATN>& core,
        ...
        LOGV2_DEBUG(0, 2, "TOTDB start to commit txn", "txn_id"_attr = core->txn_id_,
                    "commit_ts"_attr = core->commit_ts_);
        // Update Active Txns
        ...
        if (update_durable_ts) {
            while (candidate_durable_timestamp > prev_durable_timestamp) {
                ...
                if (committed_max_ts_.compare_exchange_strong(prev_durable_timestamp,
        ...
        while (stripe_keys_iter != stripe_commit_keys_map.end()) {
            ...
            for (auto& key : stripe_keys_iter->second) {
                committed_keys_.AddKeyInLock(key, core->commit_txn_id_, core->prepare_ts_,
                                             ...
                                             &current_conflict_bytes_);
                uncommitted_keys_.RemoveKeyInLock(key, stripe_keys_iter->first,
                                                  &current_conflict_bytes_);
            }
        ...
        LOGV2_DEBUG(0, 2, "TOTDB end commit txn", "txn_id"_attr = core->txn_id_,
                    "cid"_attr = core->commit_txn_id_, "commit_ts"_attr = core->commit_ts_);
    ...
    Status TOTransactionDBImpl::RollbackTransaction(const std::shared_ptr<ATN>& core,
        ...
        LOGV2_DEBUG(0, 2, "TOTDB end rollback txn", "txn_id"_attr = core->txn_id_);
    ...
    Status TOTransactionDBImpl::SetTimeStamp(const TimeStampType& ts_type, const RocksTimeStamp& ts,
                                             bool force) {
        if (ts_type == kCommitted) {
            ...
            if ((oldest_ts_ != nullptr && *oldest_ts_ > ts) && !force) {
                return Status::InvalidArgument("kCommittedTs should not be less than kOldestTs");
            }
        ...
        if (ts_type == kOldest) {
            ...
            uint64_t original = 0;
            {
                ...
                if ((oldest_ts_ != nullptr && *oldest_ts_ > ts) && !force) {
                    LOGV2_DEBUG(0, 2, "oldestTs can not travel back, keep oldest_ts",
                                "oldest_ts"_attr = *oldest_ts_, "target"_attr = ts);
                    return Status::OK();
                }
            ...
            if (force) {
                LOGV2_DEBUG(0, 2, "kOldestTs force set", "from"_attr = original, "to"_attr = ts);
            }
            auto pin_ts = ts;
            {
                std::shared_lock<std::shared_mutex> rl(read_ts_mutex_);
            ...
            LOGV2_DEBUG(0, 2, "TOTDB set oldest ts", "type"_attr = static_cast<int>(ts_type),
                        "value"_attr = pin_ts);
            return Status::OK();
        }
        ...
        if (ts_type == kStable) {
            ...
            auto s = Put(write_ops, DefaultColumnFamily(), stable_ts_key_,
                         rocksdb::Slice(ts_buf, sizeof(ts_buf)),
                         rocksdb::Slice(val_buf, sizeof(val_buf)));
            if (!s.ok()) {
                return s;
            }
            LOGV2_DEBUG(0, 2, "TOTDB set stable ts", "type"_attr = static_cast<int>(ts_type),
                        "value"_attr = ts);
            return Status::OK();
        }
    ...
    Status TOTransactionDBImpl::QueryTimeStamp(const TimeStampType& ts_type,
        ...
        if (ts_type == kOldest) {
            ...
            auto s =
                GetRootDB()->GetFullHistoryTsLow(GetRootDB()->DefaultColumnFamily(), &ts_holder);
            if (!s.ok()) {
                return s;
            }
    ...
    Status TOTransactionDBImpl::Stat(TOTransactionStat* stat) {
        ...
        stat->max_conflict_bytes = max_conflict_bytes_.load(std::memory_order_relaxed);
        ...
        stat->update_max_commit_ts_times =
            update_max_commit_ts_times_.load(std::memory_order_relaxed);
        stat->update_max_commit_ts_retries =
            update_max_commit_ts_retries_.load(std::memory_order_relaxed);
