...
#define ASSERT_ROCKS_OK(EXPRESSION) \
    ASSERT_EQUALS(::mongo::Status::OK(), ::mongo::rocksToMongoStatus(EXPRESSION))
#define ASSERT_ROCKS_NOK(EXPRESSION) \
    ASSERT_NOT_EQUALS(::mongo::Status::OK(), ::mongo::rocksToMongoStatus(EXPRESSION))
    ...
    private:
        void Open() {
            Status s =
                TOTransactionDBImpl::Open(options, txndb_options, dbname, kStablePrefix, &txn_db);
            invariant(s.ok());
            invariant(txn_db != nullptr);
        }

        void Open(Options newOptions) {
            Status s = TOTransactionDBImpl::Open(newOptions, txndb_options, dbname, kStablePrefix,
                                                 &txn_db);
            invariant(s.ok());
            invariant(txn_db != nullptr);
        }
    ...
    TEST_F(TOTransactionTest, PORT_WT_TEST_TIMESTAMP14_TEST_ALL_DURABLE) {
        ...
        txn1 = std::unique_ptr<TOTransaction>(txn_db->BeginTransaction(write_options, txn_options));
    ...
    TEST_F(TOTransactionTest, PORT_WT_TEST_TIMESTAMP14_TEST_ALL_DURABLE_OLD) {
        ...
        ASSERT_ROCKS_OK(txn1->Commit());
        RocksTimeStamp all_committed_ts;
        ASSERT_TRUE(txn_db->QueryTimeStamp(kAllCommitted, &all_committed_ts).IsNotFound());
        ...
        txn1 = std::unique_ptr<TOTransaction>(txn_db->BeginTransaction(write_options, txn_options));
        txn2 = std::unique_ptr<TOTransaction>(txn_db->BeginTransaction(write_options, txn_options));
    ...
    TEST_F(TOTransactionTest, PrepareCommitPointRead) {
        ...
        std::string value;
        auto txnW =
            std::unique_ptr<TOTransaction>(db_imp->BeginTransaction(write_options, txn_options));
    ...
    TEST_F(TOTransactionTest, PORT_WT_TEST_PREPARE_05) {
        ...
        txn = std::unique_ptr<TOTransaction>(txn_db->BeginTransaction(write_options, txn_options));
    ...
    TEST_F(TOTransactionTest, PORT_WT_TEST_PREPARE_CURSOR_01) {
        ...
        auto txn_prepare =
            std::unique_ptr<TOTransaction>(db_imp->BeginTransaction(write_options, txn_options));
        auto iter_prepare = std::unique_ptr<Iterator>(txn_prepare->GetIterator(read_options));
        ...
        auto iter_before_ts = std::unique_ptr<Iterator>(txnR_before_ts->GetIterator(read_options));

        // Txn for timestamped reads between prepare timestamp and commit timestamp.
        auto txnR_between_ts =
            std::unique_ptr<TOTransaction>(db_imp->BeginTransaction(write_options, txn_options));
        ...
        auto iter_after_ts = std::unique_ptr<Iterator>(txnR_after_ts->GetIterator(read_options));
        ...
        txnR_before_ts =
            std::unique_ptr<TOTransaction>(db_imp->BeginTransaction(write_options, txn_options));
        ASSERT_ROCKS_OK(txnR_before_ts->SetReadTimeStamp(50));
        iter_before_ts = std::unique_ptr<Iterator>(txnR_before_ts->GetIterator(read_options));

        txnR_between_ts =
            std::unique_ptr<TOTransaction>(db_imp->BeginTransaction(write_options, txn_options));
        ASSERT_ROCKS_OK(txnR_between_ts->SetReadTimeStamp(150));
        iter_between_ts = std::unique_ptr<Iterator>(txnR_between_ts->GetIterator(read_options));

        txnR_after_ts =
            std::unique_ptr<TOTransaction>(db_imp->BeginTransaction(write_options, txn_options));
        ASSERT_ROCKS_OK(txnR_after_ts->SetReadTimeStamp(250));
        iter_after_ts = std::unique_ptr<Iterator>(txnR_after_ts->GetIterator(read_options));
    ...
    TEST_F(TOTransactionTest, ResetSnapshot) {
        ...
        std::unique_ptr<TOTransaction> txn1(txn_db->BeginTransaction(write_options, txn_options));
        std::unique_ptr<TOTransaction> txn2(txn_db->BeginTransaction(write_options, txn_options));
