...
namespace rocksdb {
    ...
    private:
        ...
        using PMAP = std::map<TxnKey, std::list<std::shared_ptr<TOTransactionImpl::ActiveTxnNode>>>;
        PMAP map_;
    ...
    public:
        TOTransactionDBImpl(DB* db, const TOTransactionDBOptions& txn_db_options, bool read_only,
            ...
            num_stripes_ =
                (txn_db_options.num_stripes > 0) ? txn_db_options.num_stripes : DEFAULT_NUM_STRIPES;
        ...
        Status SetCommitTimeStamp(const std::shared_ptr<ATN>& core, const RocksTimeStamp& timesamp);

        Status SetDurableTimeStamp(const std::shared_ptr<ATN>& core,
                                   const RocksTimeStamp& timesamp);

        Status AddReadQueue(const std::shared_ptr<ATN>& core, const RocksTimeStamp& ts);
        ...
        Status GetConsiderPrepare(const std::shared_ptr<ATN>& core, ReadOptions& options,
                                  ColumnFamilyHandle* column_family, const Slice& key,
                                  std::string* value);

        Iterator* NewIteratorConsiderPrepare(const std::shared_ptr<ATN>& core,
                                             ColumnFamilyHandle* column_family, Iterator* db_iter);
        ...
        using KeyModifyHistory = std::tuple<TransactionID, RocksTimeStamp, RocksTimeStamp>;
    ...
    protected:
        ...
        class BackgroundCleanJob {
            ...
            enum ThreadState { kRunning, kStopped };

            ThreadState thread_state_;

        public:
            BackgroundCleanJob() : txnid_(0), ts_(0) { thread_state_ = kRunning; }

            ~BackgroundCleanJob() {}

            Status SetCleanInfo(const TransactionID& txn_id, const RocksTimeStamp& time_stamp);

            bool IsRunning();

            bool NeedToClean(TransactionID* txn_id, RocksTimeStamp* time_stamp);

            void FinishClean(const TransactionID& txn_id, const RocksTimeStamp& time_stamp);
    ...
    private:
        ...
        public:
            ...
            Status CheckKeyAndAddInLock(const TxnKey& key, const TransactionID& txn_id,
                                        const size_t& stripe_num, uint64_t max_mem_usage,
                                        std::atomic<int64_t>* mem_usage);
