...
#define MACRO_REFETCH_RETURN_ON_FOUND_ADVANCE_CONTINUE_ON_NOT_FOUND()                   \
    do {                                                                                \
        if (pmap_val->commit_txn_id_ < core_->txn_id_) {                                \
            ...
            auto getStatus =                                                            \
                core_->write_batch_.GetFromBatchAndDB(db_, read_opt, cf_, key_, &val_); \
    ...
    Slice PrepareMapIterator::key() const { return Slice(pos_.data(), pos_.size()); }

    const std::shared_ptr<ATN>& PrepareMapIterator::value() const { return val_; }

    TOTransaction::TOTransactionState PrepareMapIterator::valueState() const { return val_state_; }
    ...
    PrepareMergingIterator::PrepareMergingIterator(
        std::unique_ptr<Iterator> base_iterator, std::unique_ptr<PrepareMapIterator> pmap_iterator)
        : forward_(true),
    ...
    bool PrepareMergingIterator::BaseValid() const { return base_iterator_->Valid(); }

    bool PrepareMergingIterator::DeltaValid() const { return delta_iterator_->Valid(); }
    ...
    Slice PrepareMergingIterator::key() const {
        return current_at_base_ ? base_iterator_->key() : delta_iterator_->key();
    }
    ...
    ShadowValue PrepareMergingIterator::value() const {
        ...
        // TODO(wolfkdy): we need WriteBatchWithIndexIterator's timestamp() interface for
        // sanity-check.
        if (equal_keys_) {
    ...
    void PrepareMergingIterator::Next() {
        ...
        if (!forward_) {
            ...
            if (DeltaValid() && BaseValid()) {
                if (comparator_->Equal(delta_iterator_->key(), base_iterator_->key())) {
                    equal_keys_ = true;
                }
            }
        }
    ...
    void PrepareMergingIterator::AssertInvariants() {
        ...
        int compare = comparator_->Compare(delta_iterator_->key(), base_iterator_->key());
        (void)compare;
        if (forward_) {
        ...
        while (true) {
            ...
            if (!BaseValid()) {
                ...
                int compare = (forward_ ? 1 : -1) *
                              comparator_->Compare(delta_iterator_->key(), base_iterator_->key());
    ...
    void PrepareFilterIterator::Next() {
        ...
        // ---[a]---[CURRENT(b)]----[c]---------- pmap:
        // ---[a]-------------------[c(CURRENT)]-
        bool next_may_read_staled_prepared = forward_ && sval_.has_base && !sval_.has_prepare;
        forward_ = true;
        AdvanceInputNoFilter();
        UpdateCurrent();
    ...
    void PrepareFilterIterator::UpdatePrepareState() {
        invariant(sval_.prepare_value_state == TOTransaction::TOTransactionState::kPrepared);
