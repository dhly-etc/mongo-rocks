...
namespace rocksdb {
    ...
    bool IsPrepareConflict(const Status& s) {
        return s.code() == Status::Code::kInvalidArgument &&
               s.subcode() == Status::SubCode::kNone &&
    ...
    TransactionID TOTransactionImpl::GenTxnID() { return txn_id_counter_.fetch_add(1); }

    TOTransactionImpl::TOTransactionImpl(TOTransactionDB* txn_db, const WriteOptions& write_options,
    ...
    Status TOTransactionImpl::SetPrepareTimeStamp(const RocksTimeStamp& timestamp) {
        if (core_->state_ != kStarted) {
            return Status::NotSupported("this txn is prepared or committed or rollback");
        }
        ...
        if (core_->commit_ts_set_) {
            return Status::NotSupported("should not have been set before the prepare timestamp");
        }
    ...
    Status TOTransactionImpl::Prepare() {
        ...
        if (core_->commit_ts_set_) {
            return Status::NotSupported("commit ts should not have been set when prepare");
        }
    ...
    WriteBatchWithIndex* TOTransactionImpl::GetWriteBatch() { return &(core_->write_batch_); }
    ...
    Status TOTransactionImpl::Get(ReadOptions& options, ColumnFamilyHandle* column_family,
        ...
        if (written_keys_.find(txn_key) != written_keys_.end()) {
            return GetWriteBatch()->GetFromBatchAndDB(db_, options, column_family, key, value);
        }

        return txn_db_impl_->GetConsiderPrepare(core_, options, column_family, key, value);
    }

    Status TOTransactionImpl::Get(ReadOptions& options, const Slice& key, std::string* value) {
    ...
    Iterator* TOTransactionImpl::GetIterator(ReadOptions& read_options,
        ...
        return txn_db_impl_->NewIteratorConsiderPrepare(core_, column_family, db_iter);
    }
